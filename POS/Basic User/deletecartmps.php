...
//log infor for admin loginclude('../connect.php');
				...
				$result = $db->prepare("SELECT * FROM users WHERE usertype='$names' ORDER BY id DESC LIMIT 1 ");
				$result->execute();
...
$result = $db->prepare("SELECT * FROM sales_order WHERE id='$id' ORDER BY id DESC LIMIT 1 ");
				$result->execute();
				for($i=0; $row = $result->fetch(); $i++){
                $nd=$row['cart_details'];
